...
class Solution:
    def numberOfPowerfulInt(self, start: int, finish: int, limit: int, s: str) -> int:
        ...
        def count(right_limit : str, suffix : str) -> int:
            ...
            while True:
                if i == valid_size_bound: # strings with length == len(right_limit), we need to check whether they exceed right_limit
                    if right_limit[i:] >= suffix:
                        res += 1
                else:
                    res += cnt[len(right_limit) - i - 1] * (min(limit, int(right_limit[i]) - 1) + (i > 0)) # we can use 0 if the current index not the leftmost
                i += 1
