...
class Solution:
	def getWordsInLongestSubsequence(self, words: List[str], groups: List[int]) -> List[str]:
		...
		for i in range(n):
			for j in range(i):
				if len(words[i]) != len(words[j]) or groups[i] == groups[j] or calcHamDis(words[i], words[j]) != 1:
					continue
	...
	def getWordsInLongestSubsequence(self, words: List[str], groups: List[int]) -> List[str]:
		...
		for i in range(n):
			for str_len in range(max_str_len):  # length range from 0 to 10 -> O(1)
				if len(words[i]) != str_len + 1:  # if the current string of diff length -> skip
					continue

				if groups[i] == cur_groups[str_len]:  # if current string belongs to the same previous group -> skip
					continue

				if len(seqs[str_len]) == 0 or calcHamDis(seqs[str_len][len(seqs[str_len]) - 1], words[i]) == 1:
