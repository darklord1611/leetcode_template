...
class Solution:
    def minSum(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        if num_zeros[0] > 0 and num_zeros[1] > 0:
            return max(
                total_sums[0] + num_zeros[0], total_sums[1] + num_zeros[1]
            )  # we need to find the maximum sum after transform all zeroes to minimum positive number(ones)

        no_zero_index = 0 if num_zeros[0] == 0 else 1
        cur_max_sum_index = 0 if total_sums[0] > total_sums[1] else 1
        if (
            no_zero_index != cur_max_sum_index
            or total_sums[cur_max_sum_index]
            < total_sums[1 - cur_max_sum_index] + num_zeros[1 - no_zero_index]
        ):
            return -1
